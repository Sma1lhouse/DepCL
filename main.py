...
from src.model.DCRNN.DCRNN_model_v2 import DCRNNModel
...
from src.model.model import TrafficStream_Model, STKEC_Model, EAC_Model, Universal_Model
...
def main(args):
    ...
    for year in range(args.begin_year, args.end_year+1):  # Iterate through each year from the start year to the end year
        ...
        if year == args.begin_year and args.load_first_year:  # If it is the first year and you need to skip the first year, the model has been trained and does not need to be retrained
            ...
            test_loader = DataLoader(SpatioTemporalDataset(inputs, "test"), batch_size=args.batch_size, shuffle=False, pin_memory=True, num_workers=4)
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--first_year_model_path", type = str, default = "log/PEMS/eac-43/2011/14.8533.pkl", help='specify a pretrained model root')
    # parser.add_argument("--first_year_model_path", type = str, default = "log/PEMS/eac-43/2014/15.84.pkl", help='specify a pretrained model root')

    parser.add_argument("--first_year_model_path", type = str, default = "log/AIR/eac-43/2016/15.2588.pkl", help='specify a pretrained model root')

    # parser.add_argument("--first_year_model_path", type = str, default = "log/PEMS/eac-43/2011/14.1232.pkl", help='specify a pretrained model root')
    ...
    ''' revise external models'''
    from src.model.GWN.gwn_tsformer import GraphWaveNet
    from src.model.DLinear.DLinear_revise import DLinear
    from src.model.DCRNN.DCRNN_model_v2 import DCRNNModel

    vars(args)["methods"] = {'TrafficStream': TrafficStream_Model, 'STKEC': STKEC_Model, 'EAC': EAC_Model, 'Universal': Universal_Model,
                             'GWN': GraphWaveNet, 'DLinear': DLinear, 'DCRNN': DCRNNModel
